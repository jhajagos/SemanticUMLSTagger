...
def export_fragments_to_csv(alignment_dict_json_name, alignment_csv_file_name, field_name_for_id="sui", table_name=None,
    ...
    if export_json_file_name is not None:
        add_export_to_json_file(export_json_file_name, table_name, alignment_csv_file_name,
                                data_types_dict={"fragment": "VarChar(1023)", field_name_for_id: "VarChar(255)"},
...
def export_sui_info_to_csv(sui_info_json_name, table_export_name, sui_info_csv_file_name, export_json_file_name=None):
    ...
    column_data_dict = {"sui": "VarChar(255)", "umls_string": "VarChar(1023)",
                        "umls_str_original": "VarChar(1023)", "fragment": "Varchar(1023)"}
...
def postgres_export_from_json_file(json_file_name, schema="public"):
    sql_script = ""
    ...
    for table in export_tables_dict:
        sql_script += 'drop table if exists "%s"."%s";\n\n' % (schema, table)

        table_information = export_tables_dict[table]
        sql_script += 'create table "%s"."%s" (\n' % (schema, table)
        ...
        sql_script += '''/* copy "%s"."%s" from '%s' WITH DELIMITER ',' CSV HEADER; */ \n''' % (schema, table, full_path_to_csv_import_file_name)
        sql_script += '''/* \\copy "%s"."%s" from '%s' WITH DELIMITER ',' CSV HEADER; */ \n\n''' % (schema, table, full_path_to_csv_import_file_name)
        ...
        for index_to_create in indexes_to_create:
            ...
            sql_script += """create index %s on "%s"."%s"("%s");\n""" % (index_name, schema, table, index_to_create)
            i += 1

    return sql_script


def mysql_export_from_json_file(json_file_name, local="LOCAL"):
    ...
    for table in export_tables_dict:
        ...
        sql_script += "LOAD DATA %s  INFILE '%s' INTO TABLE `%s` \n" % (local, full_path_to_csv_import_file_name, table)
...
def main(json_config_file, generate_fragments=True):
    ...
    if database_type == "mysql":
        ...
        sql_script = postgres_export_from_json_file(json_file_dict_name, schema)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        with open("example.json", "w") as fw:
            json.dump(example_config, fw, indent=4, separators=(',', ': '))
